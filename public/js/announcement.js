const Announcement = {
  ...
  _loadVoices() {
    ...
    this._koVoice = this._voices.find(v => v.lang.startsWith('ko')) || null;
  },
  ...
  render() {
    ...
    document.getElementById('content').innerHTML = `
      <div class="card" style="max-width:700px">
        ...
        <div style="font-size:12px;color:#94a3b8;margin-bottom:20px">내용을 입력하고 방송 버튼을 누르면 음성으로 읽어드립니다.</div>
          ...
          <div style="display:flex;flex-wrap:wrap;gap:6px">
            ${presets.map(p => `
              <button onclick="Announcement.setPreset(${JSON.stringify(p.text).replace(/"/g,'&quot;')})"
                style="padding:5px 12px;border:1px solid #93c5fd;border-radius:20px;background:#eff6ff;
                       color:#1d4ed8;font-size:12px;font-weight:500;cursor:pointer">
                ${p.label}
              </button>`).join('')}
          </div>
        </div>
        ...
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:20px;flex-wrap:wrap">
          <div style="display:flex;align-items:center;gap:6px">
            <label style="font-size:12px;font-weight:600;color:#64748b">속도</label>
            <input type="range" id="ann-rate" min="0.6" max="1.4" step="0.1" value="0.9"
              oninput="document.getElementById('ann-rate-val').textContent=this.value"
              style="width:90px">
            <span id="ann-rate-val" style="font-size:12px;color:#475569;min-width:24px">0.9</span>
          </div>
          <div style="display:flex;align-items:center;gap:6px">
            <label style="font-size:12px;font-weight:600;color:#64748b">음량</label>
            <input type="range" id="ann-vol" min="0.1" max="1" step="0.1" value="1"
              ...
              style="width:90px">
            <span id="ann-vol-val" style="font-size:12px;color:#475569;min-width:24px">1</span>
          </div>
        </div>

        <div style="display:flex;gap:10px">
          <button id="ann-play-btn" onclick="Announcement.speak()"
            style="flex:1;padding:14px;background:#2563eb;color:#fff;border:none;border-radius:10px;
                   font-size:15px;font-weight:700;cursor:pointer;letter-spacing:0.03em">
            ▶ 방송 시작
          </button>
          <button onclick="Announcement.stop()"
            style="padding:14px 20px;background:#f1f5f9;color:#475569;border:1px solid #cbd5e1;
  ...
  speak() {
    ...
    if (this._koVoice) utter.voice = this._koVoice;
    ...
    utter.onerror = () => {
      this._setStatus('오류가 발생했습니다. 브라우저가 음성을 지원하는지 확인해주세요.', '#ef4444');
      const btn = document.getElementById('ann-play-btn');
      if (btn) { btn.style.background = '#2563eb'; btn.textContent = '▶ 방송 시작'; }
    };
