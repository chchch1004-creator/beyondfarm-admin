...
async function init() {
  ...
  tables.push(`CREATE TABLE IF NOT EXISTS fcm_tokens (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id INTEGER NOT NULL,
    token TEXT NOT NULL,
  ...
  tables.push(`CREATE TABLE IF NOT EXISTS user_settings (
    user_id INTEGER NOT NULL,
    key TEXT NOT NULL,
    value TEXT NOT NULL,
    updated_at TEXT NOT NULL DEFAULT (datetime('now','localtime')),
  ...
  if (!existingCL17.rows[0]?.cnt) {
    // 텐트 배정: 예약인원 많을수록 낮은 번호(A에 가까운) 텐트에 배정
    // 불멍세트: 오희경(15시), 신선미(15시), 노연수(19시) 3명만
    // two_time — 11 15: 강다원,김해리,박영재,박현민,송유나,이선규,조정인,함승희 / 15 19: 고승찬,신선미,오희경,이홍구,하인혜
    ...
    const cl17_11 = {
      ...
      // M텐트(0-5: tent4, 6-11: tent2): S+M 12명, 예약인원 내림차순
      tent4:[
        R(0,'M','이유나',6,'2','2','',''),
        R(1,'M','지정은',5,'1','2','',''),
        R(2,'M','박현민',5,'2','3','','','11 15'),
        R(3,'M','전은선',5,'','','',''),
        R(4,'S','서원진',5,'1','1','',''),
        R(5,'M','송영석',5,'','','',''),
      ],
      tent2:[
        ...
        R(7,'M','김유덕',5,'','','',''),
        R(8,'M','정이진',4,'2','','',''),
        R(9,'M','송유나',4,'2','','','2','11 15'),
        R(10,'S','이은주',4,'1','2','',''),
        R(11,'S','박성호',3,'1','1','',''),
      ],
      // L텐트(A-S): L제품 13명, 예약인원 내림차순
      tent8:[
        R('A','L','이선규',12,'4','2','','','11 15'),
        R('B','L','조정인',11,'4','7','','','11 15'),
        R('C','L','박영재',11,'','4','','','11 15'),
        R('D','L','김해리',10,'4','3','','','11 15'),
        R('E','L','한보라',10,'4','3','',''),
        R('F','L','신현주',9,'','','',''),
        R('G','L','최우철',9,'2','3','',''),
        R('H','L','박선주',9,'3','2','',''),
        R('J','L','함승희',9,'4','2','','','11 15'),
        R('K','L','강다원',8,'4','2','','','11 15'),
        R('L','L','정진희',7,'2','4','',''),
        R('P','L','안다영',7,'2','4','',''),
        R('S','L','송민경',7,'4','','',''),
      ],
    ...
    const cl17_15 = {
      ...
      // M텐트: 박현민→2번(11시와 동일), 송유나→9번(11시와 동일), 나머지 예약인원 내림차순
      tent4:[
        R(0,'M','이의재',6,'2','2','',''),
        R(1,'S','김미란',6,'3','','',''),
        R(2,'M','박현민',5,'','','','','11 15'),  // 11시 2번 유지
        R(3,'M','하인혜',6,'2','2','','','15 19'),
        R(4,'M','박규리',5,'1','3','',''),
        R(5,'M','고승찬',4,'','','','','15 19'),
      ],
      tent2:[
        R(6,'M','이태훈',4,'2','2','',''),
        R(7,'M','김복준',4,'','4','',''),
        R(8,'M','신선미',4,'1','1','o','','15 19'),
        R(9,'M','송유나',4,'','','','','11 15'),  // 11시 9번 유지
        R(10,'S','주소연',3,'','','',''),
        R(11,'S','김옥희',3,'','2','',''),
      ],
      // L텐트: 이선규→A, 조정인→B, 박영재→C, 김해리→D, 함승희→J, 강다원→K (11시 동일)
      tent8:[
        R('A','L','이선규',12,'','','','','11 15'),  // 11시 A 유지
        R('B','L','조정인',11,'','','','','11 15'),  // 11시 B 유지
        R('C','L','박영재',7,'6','','','','11 15'),  // 11시 C 유지
        R('D','L','김해리',16,'1','3','','','11 15'), // 11시 D 유지
        R('E','L','김유미',12,'','','',''),
        R('F','L','임일해',11,'4','4','',''),
        R('G','L','오희경',9,'3','','o','','15 19'),
        R('H','L','강지연',7,'','','',''),
        R('J','L','함승희',9,'','','','','11 15'),  // 11시 J 유지
        R('K','L','강다원',8,'','','','','11 15'),  // 11시 K 유지
        R('L','L','이홍구',7,'','','','','15 19'),
        R('P','L','최예솔',7,'','','',''),
        R('S','L','고창민',7,'','','',''),
      ],
    ...
    const cl17_19 = {
      summary:{}, extra:[],
      // M텐트: 신선미→8번(15시와 동일), 나머지 예약인원 내림차순
      tent4:[
        R(0,'M','백남주',4,'','','',''),
        R(1,'M','손광훈',4,'','','',''),
        R(2,'M','강고은',4,'','','',''),
      ...
      tent2:[
        R(6,'S','배성흥',2,'','','',''),
        R(7,'S','조재현',2,'','','',''),
        R(8,'M','신선미',4,'','','','','15 19'),  // 15시 8번 유지
        R(9,'S','김덕환',2,'','','',''),
        R(10,'S','김가영',2,'','','',''),
        R(11,'S','구태성',2,'','2','',''),
      ],
      // L텐트: 오희경→G(15시와 동일), 이홍구→L(15시와 동일), 나머지 예약인원 내림차순
      tent8:[
        R('A','L','박건표',9,'','','',''),
        R('B','M','하인혜',6,'','','','','15 19'),
        R('C','M','김지영',6,'','','',''),
        ...
        R('E','M','이지운',4,'','','',''),
        R('F','M','주영우',4,'','','',''),
        R('G','L','오희경',9,'','','','','15 19'),  // 15시 G 유지
        R('H','M','조우민',4,'','','',''),
        E('J'), E('K'),
        R('L','L','이홍구',7,'','','','','15 19'),  // 15시 L 유지
        E('P'), E('S'),
      ],
    };
