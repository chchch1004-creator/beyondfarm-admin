...
async function init() {
  ...
  if (existingCL17row.rows[0]) {
    try {
      ...
      const t4 = d.tent4?.[0];
      // 불멍이 잘못 되어있거나(이유나는 불멍없음) 또는 tent8[0]이 이선규가 아니면 재시드
      if ((t4 && t4.bulmung === 'o') || d.tent8?.[0]?.name !== '이선규') {
  ...
  if (!existingCL17.rows[0]?.cnt) {
    ...
    const cl17_15 = {
      ...
      // M텐트: S+M 12명, 예약인원 내림차순
      tent4:[
        R(0,'M','이의재',6,'2','2','',''),
        R(1,'S','김미란',6,'3','','',''),
        R(2,'M','하인혜',6,'2','2','','','15 19'),
        R(3,'M','박현민',5,'','','','','11 15'),
        R(4,'M','박규리',5,'1','3','',''),
        R(5,'M','고승찬',4,'','','','','15 19'),
      ],
      tent2:[
        ...
        R(7,'M','송유나',4,'','','','','11 15'),
        R(8,'M','김복준',4,'','4','',''),
        R(9,'M','신선미',4,'1','1','o','','15 19'),
        R(10,'S','주소연',3,'','','',''),
        R(11,'S','김옥희',3,'','2','',''),
      ],
      // L텐트: L제품 13명, 예약인원 내림차순
      tent8:[
        R('A','L','김해리',16,'1','3','','','11 15'),
        R('B','L','김유미',12,'','','',''),
        R('C','L','이선규',12,'','','','','11 15'),
        R('D','L','조정인',11,'','','','','11 15'),
        R('E','L','임일해',11,'4','4','',''),
        R('F','L','오희경',9,'3','','o','','15 19'),
        R('G','L','함승희',9,'','','','','11 15'),
        R('H','L','강다원',8,'','','','','11 15'),
        R('J','L','강지연',7,'','','',''),
        R('K','L','이홍구',7,'','','','','15 19'),
    ...
    const cl17_19 = {
      summary:{}, extra:[],
      // S+M 18명 → 상위 6명(예약인원 많은 순)은 L텐트로, 하위 12명은 M텐트
      // M텐트: 백남주(4),손광훈(4),강고은(4),노연수(4,불멍),양준황(4),신선미(4,15 19),이정환(3),배성흥(2),조재현(2),김덕환(2),김가영(2),구태성(2)
      tent4:[
        R(0,'M','백남주',4,'','','',''),
        R(1,'M','손광훈',4,'','','',''),
        R(2,'M','강고은',4,'','','',''),
      ...
      // L텐트: L제품 3명 + S/M 오버플로우 6명(예약인원 내림차순), 합쳐서 정렬
      tent8:[
        R('A','L','박건표',9,'','','',''),
        R('B','L','오희경',9,'','','','','15 19'),
        R('C','L','이홍구',7,'','','','','15 19'),
