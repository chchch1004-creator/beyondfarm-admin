...
async function init() {
  ...
  // 티켓 예약인원수가 비어있으면 데이터가 잘못된 것이므로 재시드
  ...
  const existingCL17 = await client.execute({ sql: "SELECT COUNT(*) as cnt FROM checklist_data WHERE date='2026-07-17'", args: [] });
  if (!existingCL17.rows[0]?.cnt) {
    // two_time: 복수 타임 예약자 — 11 15: 강다원,김해리,박영재,박현민,송유나,이선규,조정인,함승희 / 15 19: 고승찬,신선미,오희경,이홍구,하인혜
    const R = (no, prod, name, reserved, child_pool, adult_pool, bulmung, play, two_time) => ({
      tent_no: String(no), product: prod||'', visit_count:'', name: name||'', reserved: reserved||'',
      actual:'', two_time: two_time||'', play: play||'', child_pool: child_pool||'', adult_pool: adult_pool||'',
      bulmung: bulmung||'', adult_only:'', extra_hour:'', memo:''
    });
    const E = (no) => R(no,'','','','','','','');
    const TNOS = ['A','B','C','D','E','F','G','H','J','K','L','P','S'];

    const cl17_11 = {
      summary:{}, extra:[
        R('','','권일남',4,'2','2','',''), R('','','권일남',1,'1','','',''), R('','','방승완',5,'3','','',''),
      ],
      tent4:[
        R(0,'M','정이진',4,'2','','o',''), R(1,'M','지정은',5,'1','2','o',''),
        R(2,'M','박현민',5,'2','3','o','','11 15'), R(3,'M','전은선',5,'','','o',''),
        R(4,'M','송영석',5,'','','o',''), R(5,'M','송유나',4,'2','','o','2','11 15'),
      ],
      tent2:[
        R(6,'M','이유나',6,'2','2','o',''), R(7,'M','김유덕',5,'','','o',''),
        R(8,'S','서원진',5,'1','1','o',''), R(9,'S','김숙현',5,'2','3','o','2'),
        R(10,'S','이은주',4,'1','2','o',''), R(11,'S','박성호',3,'1','1','o',''),
      ],
      tent8:[
        R('A','L','신현주',9,'','','o',''), R('B','L','김해리',10,'4','3','o','','11 15'),
        R('C','L','최우철',9,'2','3','o',''), R('D','L','조정인',11,'4','7','o','','11 15'),
        R('E','L','이선규',12,'4','2','o','','11 15'), R('F','L','박영재',11,'','4','o','','11 15'),
        R('G','L','한보라',10,'4','3','o',''), R('H','L','정진희',7,'2','4','o',''),
        R('J','L','박선주',9,'3','2','o',''), R('K','L','함승희',9,'4','2','o','','11 15'),
        R('L','L','강다원',8,'4','2','o','','11 15'), R('P','L','안다영',7,'2','4','o',''),
        R('S','L','송민경',7,'4','','o',''),
      ],
    };

    const cl17_15 = {
      summary:{}, extra:[ R('','','오유미',2,'','2','','') ],
      tent4:[
        R(0,'M','박현민',5,'','','o','','11 15'), R(1,'M','이의재',6,'2','2','o',''),
        R(2,'M','박규리',5,'1','3','o',''), R(3,'M','고승찬',4,'','','o','','15 19'),
        R(4,'M','이태훈',4,'2','2','o',''), R(5,'M','송유나',4,'','','o','','11 15'),
      ],
      tent2:[
        R(6,'M','하인혜',6,'2','2','o','','15 19'), R(7,'M','김복준',4,'','4','o',''),
        R(8,'M','신선미',4,'1','1','o','','15 19'), R(9,'S','주소연',3,'','','o',''),
        R(10,'S','김옥희',3,'','2','o',''), R(11,'S','김미란',6,'3','','o',''),
      ],
      tent8:[
        R('A','L','강지연',7,'','','o',''), R('B','L','김유미',12,'','','o',''),
        R('C','L','김해리',16,'1','3','o','','11 15'), R('D','L','조정인',11,'','','o','','11 15'),
        R('E','L','이선규',12,'','','o','','11 15'), R('F','L','이홍구',7,'','','o','','15 19'),
        R('G','L','박영재',7,'6','','o','','11 15'), R('H','L','임일해',11,'4','4','o',''),
        R('J','L','최예솔',7,'','','o',''), R('K','L','오희경',9,'3','','o','','15 19'),
        R('L','L','함승희',9,'','','o','','11 15'), R('P','L','강다원',8,'','','o','','11 15'),
        R('S','L','고창민',7,'','','o',''),
      ],
    };
    ...
    const cl17_19 = {
      summary:{}, extra:[],
      tent4:[
        R(0,'M','고승찬',4,'','','o','','15 19'), R(1,'M','이지운',4,'','','o',''),
        R(2,'M','주영우',4,'','','o',''), R(3,'M','하인혜',6,'','','o','','15 19'),
        R(4,'M','조우민',4,'','','o',''), R(5,'M','백남주',4,'','','o',''),
      ],
      tent2:[
        R(6,'M','손광훈',4,'','','o',''), R(7,'M','강고은',4,'','','o',''),
        R(8,'M','양준황',4,'','','o',''), R(9,'M','신선미',4,'','','o','','15 19'),
        R(10,'M','김지영',6,'','','o',''), R(11,'S','배성흥',2,'','','o',''),
      ],
      tent8:[
        R('A','L','이홍구',7,'','','o','','15 19'), R('B','L','박건표',9,'','','o',''),
        R('C','L','오희경',9,'','','o','','15 19'), R('D','S','조재현',2,'','','o',''),
        R('E','S','김덕환',2,'','','o',''), R('F','S','노연수',4,'','2','o',''),
        R('G','S','이정환',3,'','','o',''), R('H','S','김가영',2,'','','o',''),
        R('J','S','구태성',2,'','2','o',''),
        E('K'), E('L'), E('P'), E('S'),
      ],
    };

    for (const [ts, d] of [['11',cl17_11],['15',cl17_15],['19',cl17_19]]) {
      try {
        await client.execute({
          sql: "INSERT OR REPLACE INTO checklist_data (date, timeslot, data) VALUES ('2026-07-17', ?, ?)",
          args: [ts, JSON.stringify(d)]
        });
      } catch {}
    }
    console.log('7월 17일 인원체크리스트 초기 데이터 삽입 완료');
  }
